...
# COLLECTION AND CLEANING OF DATA
    ...
    for attr in range(number_attr/2):
        if attr != 8:
            ...
            size = len(data)
            for i in range(size):
                data[i,attr*2] = max(data[i,attr*2],data[i,attr*2+1])  # possession is the bigger one
            data[:,attr*2] = normalise(data[:,attr*2])
    merged_data = data[:,[i for i in range(number_attr) if i%2 == 0]]  # merge the data of home and away
    
    # Shot total,  Shot on target,  interception,  Tackles total,   Tackles success rate,  Foul commited,
...
# CLUSTERING
    ...
    for match_no in range(size):
        while root.is_root != True:  # get the real root
            root = root.parent
        root.insert(Entry(merged_data[match_no],match_no,division[match_no]))
...
#        print('Bundesliga : %d' % Bundesliga)
    ...
    def analyse(entry):  # analyse a brunch, input a node or a leaf
        components = proportionAnalysis(entry.showChildren())
        data = meanData(entry.showChildrenData())
        print('EPL : %f' % components[0])
        print('Ligue_1 : %f' % components[1])
        print('La_Liga : %f' % components[2])
        print('Serie_A : %f' % components[3])
        print('Bundesliga : %f' % components[4])
        print
        print('Shot total :%f' % data[0])
        print('Shot on target :%f' % data[1])
        print('interception :%f' % data[2])
        print('Tackles total :%f' % data[3])
        print('Tackles success rate :%f' % data[4])
        print('Foul commited :%f' % data[5])
        print('Passes key :%f' % data[6])
        print('Passes sucess :%f' % data[7])
        print('Possession :%f' % data[8])
        print('Dribble succes :%f' % data[9])
        print('Aerials total :%f' % data[10])
        print('Corners :%f' % data[11])
        print
        print
